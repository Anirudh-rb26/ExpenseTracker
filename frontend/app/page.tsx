...
export default function Dashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      const [groupsData, usersData]: [any[], any[]] = await Promise.all([api.getGroups(), api.getUsers()])
  ...
  const totalOwed = userBalances.reduce((sum, balance) => {
    const owesAmount = Object.values(balance.owes || {}).reduce((total, amount) => total + (amount || 0), 0)
    return sum + owesAmount
  }, 0)

  const totalOwedTo = userBalances.reduce((sum, balance) => {
    const owedAmount = Object.values(balance.owed_by || {}).reduce((total, amount) => total + (amount || 0), 0)
        ...
        <div className="max-w-6xl mx-auto">
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-sm text-gray-400 text-center mb-4">{getRetryMessage(error.type)}</p>
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <Button onClick={() => setShowAddExpense(true)} size="sm" disabled={!isOnline}>
                <Plus className="w-4 h-4 mr-2" />
                Add Expense
              </Button>
              <Button onClick={() => setShowCreateGroup(true)} variant="outline" size="sm" disabled={!isOnline}>
          ...
          {groups.length === 0 ? (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <Button onClick={() => setShowCreateGroup(true)} disabled={!isOnline}>
                  <Plus className="w-4 h-4 mr-2" />
                  Create Group
                </Button>
              </CardContent>
                ...
                <div className="divide-y divide-gray-200">
                  {userBalances.slice(0, 5).map((balance, index) => (
                    <div key={index} className="p-4 flex items-center justify-between">
                      ...
                      <div
                        className={`font-semibold ${(balance.net_balance || 0) >= 0 ? "text-green-600" : "text-red-600"}`}
                      >
                        {(balance.net_balance || 0) >= 0 ? "+" : ""}${(balance.net_balance || 0).toFixed(2)}
                      </div>
                    </div>
                  ))}
                </div>
